...
import { useVendorDeleteProduct } from '@/services/vendor/product';

export function ProductCard({ product }: { product: ProductCardType }) {
  const { mutate: addToCart, isPending: isCartPending } = useAddToCart();
  const deleteMutation = useVendorDeleteProduct();
  ...
  return (
    <>
      <Card className='group relative h-full overflow-hidden rounded pt-0 pb-2 transition-all duration-300 hover:shadow-md md:rounded-lg md:pb-4'>
        ...
        <div className='bg-background/80 hover:bg-background absolute top-2 right-2 z-20 flex gap-1 rounded-full p-0.5 shadow-sm backdrop-blur-sm transition-all sm:top-3 sm:right-3'>
          <Button
            variant='ghost'
            size='icon'
            onClick={(e) => {
              e.preventDefault();
              e.stopPropagation();
              router.push(`/dashboard/vendor/products/edit/${product.id}`);
            }}
          ...
          <Button
            ...
            className='h-7 w-7 rounded-full text-destructive hover:text-destructive sm:h-8 sm:w-8'
          >
